...
export const AnalyticsDashboard: React.FC = () => {
  ...
  const StatCard = ({ title, value, icon: Icon, color, subtitle }: {
    ...
    <div className="bg-slate-800/50 backdrop-blur-sm rounded-xl shadow-lg p-6 border border-slate-600">
      <div className="flex items-center justify-between">
        ...
        <div className={`p-3 rounded-lg ${color.includes('red') ? 'bg-red-900/30 border border-red-700' : 
          color.includes('blue') ? 'bg-blue-900/30 border border-blue-700' : 
          color.includes('green') ? 'bg-green-900/30 border border-green-700' : 
          color.includes('yellow') ? 'bg-yellow-900/30 border border-yellow-700' : 'bg-purple-900/30 border border-purple-700'}`}>
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900/20 to-slate-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-300">Loading disaster analytics...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900/20 to-slate-900 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-slate-300 mt-2">Real-time disaster monitoring and trend analysis</p>
            </div>
            <div className="flex items-center space-x-4">
              ...
              <select
                ...
                className="px-4 py-2 border border-slate-600 bg-slate-700 text-white rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              >
        ...
        {/* Recent Activity */}
        <div className="mt-8 bg-slate-800/50 backdrop-blur-sm rounded-xl shadow-lg p-6 border border-slate-600">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-600">
                  ...
                  <th className="text-left py-3 px-4 text-slate-300">Magnitude</th>
                  <th className="text-left py-3 px-4 text-slate-300">Affected</th>
              ...
              <tbody>
                {disasters.slice(0, 8).map((disaster) => (
                  <tr key={disaster.id} className="border-b border-slate-700 hover:bg-slate-700/30">
                    <td className="py-3 px-4 font-medium text-white">{disaster.type}</td>
                    <td className="py-3 px-4 text-slate-300">{disaster.location.name}</td>
                    <td className="py-3 px-4 text-slate-300">{disaster.magnitude.toFixed(1)}</td>
